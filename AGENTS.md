...
Conventions: factory functions + `Object.freeze` (the one exception is the trivial `VSCodeCommandAdapter` class — see **Code style** below, which otherwise rules out classes), guard clauses, dependency bags typed inline at the consumer. Both the manifest and the runtime strings are localized into 12 locales; see **Toolchain**.
...
- **Runtime bundles stay in key-parity too, and `bundle.l10n.json` is the source.** All 12 `l10n/bundle.l10n.*.json` carry exactly the keys of the English bundle, and every key is a live `l10n.t()` call site. Only the nls half was ever pinned, so a string that moved from `l10n.t()` to `console.error` left 12 orphaned translations behind and English silently ran one key short — invisible, because a key absent from English is exactly what a *missing translation* also looks like from the other direction. When a runtime string is removed or demoted, delete it from all 13 files in the same commit.
