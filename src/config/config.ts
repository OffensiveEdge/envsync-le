...
export function readConfig(configuration: Configuration): DotSyncConfig {
	...
	const fileSizeWarnBytes = Math.max(1024, Number(cfg.get('safety.fileSizeWarnBytes', 1024 * 1024)))
	const maxFilesWarn = Math.max(1, Number(cfg.get('safety.maxFilesWarn', 50)))
	const maxTotalSizeWarn = Math.max(1024 * 1024, Number(cfg.get('safety.maxTotalSizeWarn', 5 * 1024 * 1024)))
	const maxProcessingTimeWarn = Math.max(1000, Number(cfg.get('safety.maxProcessingTimeWarn', 5000)))
