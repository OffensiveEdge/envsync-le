...
describe('parseDotenvFile integration tests', () => {
	...
	it('should parse sample.env.local correctly', () => {
		...
		const expected = readFileSync(join(dataDir, 'sample.env.local.expected.txt'), 'utf-8')
			.trim()
			.split('\n')
			.filter((line) => line.trim())
	...
	it('should parse sample.env.example correctly', () => {
		...
		const expected = readFileSync(join(dataDir, 'sample.env.example.expected.txt'), 'utf-8')
			.trim()
			.split('\n')
			.filter((line) => line.trim())
	...
	it('should handle invalid.env gracefully', () => {
		...
		const expected = readFileSync(join(dataDir, 'invalid.env.expected.txt'), 'utf-8')
			.trim()
			.split('\n')
			.filter((line) => line.trim())
...
api_key=sk-67890`
		...
		expect(result.keys).toEqual(['DATABASE_URL', 'database_url', 'API_KEY', 'api_key'])
	})
...
SPECIAL_CHARS=!@#$%^&*()_+-=[]{}|;':",./<>?`
		...
		expect(result.success).toBe(true)
		expect(result.keys).toEqual(['COMPLEX_URL', 'JSON_VALUE', 'MULTILINE_VALUE', 'SPECIAL_CHARS'])
	})
})
