...
describe('detector integration tests', () => {
	...
	it('should detect in-sync files correctly', async () => {
		...
		deps.fileSystem.findFiles = vi.fn().mockResolvedValue([
			...
			{ filepath: '/project/.env.example', uri: 'file:///project/.env.example' },
		])
		deps.fileSystem.readFile = vi.fn().mockResolvedValue(sampleContent)
		deps.fileSystem.getFileStats = vi.fn().mockResolvedValue({ mtime: new Date() })
		deps.fileSystem.asRelativePath = vi.fn().mockImplementation((path: string) => path.split('/').pop() || path)
	...
	it('should detect missing keys correctly', async () => {
		...
		const localContent = readFileSync(join(dataDir, 'sample.env.local'), 'utf-8')
		...
		const envMissing = report.missingKeys.find((m) => m.filepath === '/project/.env')
	...
	it('should handle parse errors gracefully', async () => {
		...
		deps.fileSystem.findFiles = vi.fn().mockResolvedValue([{ filepath: '/project/.env', uri: 'file:///project/.env' }])
	...
	it('should handle template mode correctly', async () => {
		...
		const exampleContent = readFileSync(join(dataDir, 'sample.env.example'), 'utf-8')
		const localContent = readFileSync(join(dataDir, 'sample.env.local'), 'utf-8')
		...
		const localMissing = report.missingKeys.find((m) => m.filepath === '/project/.env.local')
