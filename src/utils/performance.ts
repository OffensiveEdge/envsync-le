...
export class PerformanceMonitor {
	...
	private readonly cache = new Map<string, { data: unknown; timestamp: number; hits: number }>()
	...
	getReport(): PerformanceReport {
		...
		const avgFileCount = recentMetrics.reduce((sum, m) => sum + m.fileCount, 0) / recentMetrics.length
		...
		const latestMetrics = recentMetrics[recentMetrics.length - 1] || this.getDefaultMetrics()
...
export class PerformanceTracker {
	...
	private readonly cache: Map<string, { data: unknown; timestamp: number; hits: number }>
	private cacheHits = 0
	 ...
	 */
	getCached<T>(key: string, compute: () => T, maxAge: number = 5 * 60 * 1000): T {
		const now = Date.now()
 ...
 */
export function getDefaultPerformanceThresholds(config: DotSyncConfig): PerformanceThresholds {
	return {
 ...
 */
export function isPerformanceAcceptable(metrics: PerformanceMetrics, thresholds: PerformanceThresholds): boolean {
	return (
...
export function getPerformanceOptimizations(
	...
	if (metrics.cacheMisses > metrics.cacheHits * 2) {
		optimizations.push(
			localize('runtime.performance.optimization.cache', 'Improve cache key strategy or increase cache size'),
		)
	}
 ...
 */
export function createPerformanceMonitor(config: DotSyncConfig): PerformanceMonitor {
 ...
 */
export function createPerformanceDebounce<T extends (...args: unknown[]) => unknown>(
	func: T,
	delay: number,
