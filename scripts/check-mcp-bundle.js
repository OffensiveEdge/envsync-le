...
send({
	...
	params: {
		...
		arguments: {"files": [{"path": ".env.example", "content": "API_URL=\nAPI_KEY=\nDEBUG="}, {"path": ".env", "content": "API_URL=http://localhost\nDEBUG=1"}], "mode": "template", "templatePath": ".env.example"},
	},
});
...
setTimeout(() => {
	...
	if (init?.serverInfo?.name !== 'envsync-le') {
	...
	if (call?.isError !== false) {
		fail('compare_env_files reported an error', JSON.stringify(call));
	}
	const data = call?.structuredContent?.data;
	const missing = data?.missingKeys ?? [];
	// Asserting the actual finding, not just "it responded" — a server that
	// answers with an empty report would otherwise pass.
	if (!missing.some((m) => m.keys.includes('API_KEY'))) {
		fail(
			'compare_env_files did not report the missing key',
			JSON.stringify(data),
		);
	}
	// Values must never cross the boundary: only key names do.
	if (JSON.stringify(data).includes('http://localhost')) {
		fail(
			'compare_env_files returned a value from a dotenv file',
			'only key names may be returned',
		);
	}
