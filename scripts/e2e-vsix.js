...
// Workspace with a known missing key: .env has DEBUG, .env.local lacks it.
...
exports.run = async function run() {
	...
	// Smoke the primary command end to end: the report must call out the
	// key missing from .env.local.
	await vscode.commands.executeCommand('envsync-le.showIssues');
	const report = vscode.workspace.textDocuments.find(
		(d) => d.languageId === 'markdown' && d.getText().includes('Sync Report'),
	);
	assert.ok(report, 'no sync report document produced');
	...
	console.log('VSIX E2E OK: report names .env.local and the missing DEBUG key');
