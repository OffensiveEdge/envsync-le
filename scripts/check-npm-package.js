 ...
 * `npx envsync-le-mcp` does, minus the registry — so it catches the failures a
...
setTimeout(() => {
	...
	if (!(data?.missingKeys ?? []).some((m) => m.keys.includes('API_KEY'))) {
		fail(
			'the installed server did not report the missing key',
			JSON.stringify(data),
		);
	}
	if (JSON.stringify(data).includes('http://localhost')) {
		fail(
			'the installed server returned a dotenv value',
