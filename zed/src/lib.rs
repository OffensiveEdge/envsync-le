//! The Zed side of EnvSync-LE.
//!
//! This crate holds no extraction logic. It resolves `envsync-le-mcp` from npm and
//! hands Zed a command that launches it over stdio — the same server the VSIX
//! bundles and the same one `npx envsync-le-mcp` runs. Keeping it a launcher is
...
const PACKAGE_NAME: &str = "envsync-le-mcp";
const SERVER_PATH: &str = "node_modules/envsync-le-mcp/server.js";

struct EnvSyncLEExtension;
...
impl zed::Extension for EnvSyncLEExtension {
    fn new() -> Self {
        Self
    }
...
zed::register_extension!(EnvSyncLEExtension);
