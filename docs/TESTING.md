# EnvSync-LE Testing Guidelines

This document outlines testing practices and policies for EnvSync-LE development.
...
```bash
bun x vitest run src/comparison/comparison.test.ts
```
...
- **Pure function tests** - Test comparison logic in isolation
- **Parsing tests** - Test .env file parsing
- **Utility tests** - Test helper functions
- **Configuration tests** - Test config validation

### Integration Tests

Located in `test/` or `src/**/*.spec.ts`:

- **File watching tests** - Test file system monitoring
- **Cross-platform tests** - Ensure case-sensitivity compatibility
- **End-to-end workflows** - Test complete sync flows

## Test Coverage Requirements

- **Minimum Coverage**: Maintain reasonable coverage across core functionality
- **Critical Paths**: All comparison logic must be tested
...
- **File system timing** - Account for file system event delays in tests
...
```typescript
// ✅ Good
it('should detect missing keys in .env files', () => {
  // ...
});
...
```typescript
it('should compare .env files', () => {
  // Arrange
  const file1 = 'KEY1=value1\nKEY2=value2';
  const file2 = 'KEY1=value1';
...
const content = readSampleFile('.env.example');
